...
\begin{tikzpicture}[x=0.85cm,y=0.85cm,>=Latex]
  \node[draw, rounded corners, minimum width=3.0cm, minimum height=1.55cm, align=center, fill=gray!8] (ambient) at (0,0) {$\C^2 \cong \R^4$\\[2pt]all nonzero spinors\\[2pt]$\ket{\psi}=\begin{pmatrix}\alpha\\ \beta\end{pmatrix}$};
  \node[draw, rounded corners, minimum width=3.0cm, minimum height=1.6cm, align=center, fill=blue!5] (spinor) at (4.2,0) {normalized reps.\\[2pt]$\{\ket{\psi}\in\C^2:\norm{\psi}=1\}$\\[2pt]$=S^3$};
  \node[draw, rounded corners, minimum width=3.2cm, minimum height=1.6cm, align=center, fill=green!5] (projective) at (8.8,0) {projective state space\\[2pt]$S^3/U(1)=\CP^1$\\[2pt]pure states modulo phase};
  \node[draw, rounded corners, minimum width=3.1cm, minimum height=1.45cm, align=center, fill=yellow!10] (orbit) at (4.2,-3.1) {one phase orbit\\[2pt]$\{e^{i\chi}\ket{\psi}:\chi\in\R\}$\\[2pt]same pure state};
  \node[draw, rounded corners, minimum width=3.2cm, minimum height=1.45cm, align=center, fill=green!12] (bloch) at (8.8,-3.1) {Bloch-sphere picture\\[2pt]$\mathbf{r}(\psi)\in S^2$\\[2pt]north: $\ket{0}$, south: $\ket{1}$};
  \draw[->, thick] (ambient) -- node[above, align=center, font=\small] {impose\\$\norm{\psi}=1$} (spinor);
  \draw[->, thick] (spinor) -- node[above, align=center, font=\small] {quotient by\\global phase $U(1)$} (projective);
  \draw[->, thick] (spinor) -- node[right, align=center, font=\small] {points on same\\phase circle} (orbit);
  \draw[->, thick] (projective) -- node[right, align=center, font=\small] {identify pure states\\with Bloch vectors} (bloch);
  \draw[->, dashed, thick] (orbit.east) -- node[below, align=center, font=\small] {each orbit collapses\\to one point} (projective.south west);
\end{tikzpicture}
\caption{The normalized-spinor picture breaks into distinct steps: start in $\C^2\cong\R^4$, restrict to the unit sphere $S^3$ of normalized representatives, group points into $U(1)$ phase orbits, and then pass to the projective pure-state space $\CP^1\cong S^2$, the Bloch sphere.}
\end{figure}

\begin{figure}[h]
\centering
\begin{tikzpicture}[x=0.85cm,y=0.85cm,>=Latex]
  \node[draw, rounded corners, minimum width=3.1cm, minimum height=1.45cm, align=center, fill=orange!8] (u2) at (0,0) {full gate\\[2pt]$U\in\U(2)$\\[2pt]includes overall phase};
  \node[draw, rounded corners, minimum width=3.1cm, minimum height=1.45cm, align=center, fill=orange!4] (class) at (4.3,0) {physical gate class\\[2pt]$[U]\in\U(2)/\U(1)$\\[2pt]phase removed};
  \node[draw, rounded corners, minimum width=3.1cm, minimum height=1.45cm, align=center, fill=orange!8] (su2) at (8.6,0) {chosen representative\\[2pt]$V\in\SU(2)$\\[2pt]unique up to $\pm V$};
  \node[draw, rounded corners, minimum width=3.1cm, minimum height=1.45cm, align=center, fill=purple!8] (quat) at (4.3,-3.0) {unit quaternion\\[2pt]$q\in\Sp(1)$\\[2pt]$\Phi(q)=V$};
  \node[draw, rounded corners, minimum width=3.2cm, minimum height=1.45cm, align=center, fill=purple!4] (so3) at (8.6,-3.0) {rotation on Bloch vectors\\[2pt]$\rho(q)\in\SO(3)$\\[2pt]$q$ and $-q$ act identically};
  \draw[->, thick] (u2) -- node[above, align=center, font=\small] {quotient by\\overall phase} (class);
  \draw[->, thick] (class) -- node[above, align=center, font=\small] {choose a\\det-one rep.} (su2);
  \draw[<->, thick] (su2) -- node[right, font=\small] {$\Phi$} (quat);
  \draw[->, thick] (quat) -- node[above, align=center, font=\small] {conjugation on\\pure quaternions} (so3);
  \draw[->, dashed, thick] (su2.south) -- node[right, align=center, font=\small] {$\pm V$ give same\\rotation action} (so3.north);
\end{tikzpicture}
\caption{The gate-representation chain also breaks into distinct layers: a physical single-qubit gate starts in $\U(2)$, passes to a phase-quotiented class, is represented by some $V\in\SU(2)$, is encoded by a unit quaternion $q\in\Sp(1)$ through $\Phi$, and finally induces a rotation in $\SO(3)$ on Bloch vectors.}
\end{figure}

\begin{figure}[h]
\centering
\begin{tikzpicture}[x=0.9cm,y=0.9cm,>=Latex]
  \draw[->, thick] (0,0) -- (2.0,0) node[below] {$\ii$ / $x$};
  \draw[->, thick] (0,0) -- (0,2.0) node[left] {$\jj$ / $y$};
  \draw[->, thick] (0,0) -- (-1.1,-1.2) node[below] {$\kk$ / $z$};
  \node[draw, rounded corners, fill=purple!3, align=left, anchor=west, text width=7.5cm, inner sep=6pt] at (2.4,1.0) {
  $q(\theta,\hat{\bloch{n}})=\cos(\theta/2)+\sin(\theta/2)\,\hat{\bloch{n}}$\\[3pt]
  $(a_1+\qvec_1)(a_2+\qvec_2)$\\
  $=(a_1a_2-\qvec_1\cdot\qvec_2)$\\
  $\quad +(a_1\qvec_2+a_2\qvec_1+\qvec_1\times\qvec_2)$};
\end{tikzpicture}
